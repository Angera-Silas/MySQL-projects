...
create table Book(BookID INT AUTO_INCREMENT, BookTitle char(100) not null
, AuthorID INT not null, CONSTRAINT PK_BookID PRIMARY KEY (BookID), CONSTRAINT FK_AuthorID FOREIGN KEY (AuthorID) REFERENCES Author(AuthorID) ON UPDATE CASCADE
ON DELETE CASCADE);

create table Borrower(BorrowerID INT AUTO_INCREMENT , ClientID INT not null ,BookID INT not null, BorrowDate date, CONSTRAINT PK_BorrowID PRIMARY KEY (BorrowerID), CONSTRAINT FK_ClientID FOREIGN KEY (ClientID) REFERENCES Client(ClientID) ON UPDATE CASCADE ON DELETE CASCADE, CONSTRAINT FK_BookID FOREIGN KEY (BookID) REFERENCES Book (BookID));
SHOW TABLES;
