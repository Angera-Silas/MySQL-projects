...
SELECT
    ...
    SUM(IN_STOCK) AS BALANCE
FROM `COMPACT_DISCS`
GROUP BY(LABEL_ID) WITH ROLLUP LIMIT 20;
...
SELECT
    ...
    SUM(IN_STOCK) AS BALANCE
FROM `COMPACT_DISCS`
GROUP BY(`LABEL_ID`) HAVING BALANCE < 20;

SELECT * FROM ARTISTS;
SELECT * FROM `CDS_IN_STOCK`;
SELECT CD_TITLE,IN_STOCK FROM COMPACT_DISCS WHERE IN_STOCK > 10 AND IN_STOCK <30;
