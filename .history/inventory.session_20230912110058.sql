...
/* The querry below shows how to use WHERE clause in combina
*/
SELECT CD_TITLE ,IN_STOCK FROM `COMPACT_DISCS` WHERE IN_STOCK = 6 OR IN_STOCK > 10 AND LABEL_ID = 835;
SELECT * FROM `COMPACT_DISCS`WHERE (`IN_STOCK`=6 OR `IN_STOCK`=27) AND (`LABEL_ID`=833 OR `LABEL_ID`=829);
SELECT CD_TITLE,IN_STOCK FROM COMPACT_DISCS WHERE IN_STOCK <> 13;
SELECT CD_TITLE FROM `COMPACT_DISCS`WHERE `IN_STOCK` < 20 AND NOT `LABEL_ID`< 810;
SELECT * FROM `CDS_IN_STOCK`;
/*a statement to demonstrate the use of group by clause*/
SELECT `LABEL_ID`,SUM(IN_STOCK) AS BALANCE FROM `COMPACT_DISCS` GROUP BY(`LABEL_ID`)
