...
WHERE `LABEL_ID` IN (
        ...
        FROM `CD_LABELS`
        WHERE
            `COMPANY_NAME` = 'Decca Record Company'
    )
...
WHERE EXISTS (
        ...
        FROM CD_LABELS
        WHERE
            COMPACT_DISCS.LABEL_ID = CD_LABELS.LABEL_ID
            AND LABEL_ID > 830
    )

SELECT LABEL_ID,COMPANY_NAME FROM CD_LABELS
WHERE LABEL_ID = ANY
(SELECT LABEL_ID FROM COMPACT_DISCS WHERE IN_STOCK > 20)